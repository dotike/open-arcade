...
@mock_s3
@mock_sts
def test_narc_dump(capsys, monkeypatch, valid_asteroid_file, s3_valid_asteroid):
    ...
    monkeypatch.setattr("sys.argv", ["pytest", "--arcade", arcade, "--bucket", target_bucket.name, "--file", narc_nginxaarf, "--JSON"])
    narc_dump.main()

    monkeypatch.setattr("sys.argv", ["pytest", "--arcade", arcade, "--bucket", target_bucket.name, "--file", narc_nginxwoof, "--JSON"])
    narc_dump.main()
